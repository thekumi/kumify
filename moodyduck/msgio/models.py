    ...
    def send(self):
        ...
        if dispatchers:
            ...
            for dispatcher in GatewayUser.objects.filter(user=self.recipient):
                response.append(
                    send_message.send_robust(
                        self.__class__, dispatcher=dispatcher.gateway, notification=self
                    )
                )
...
class TelegramPairingToken(models.Model):
    """One-time token that links a Telegram chat_id to a MoodyDuck user account."""

    user = models.OneToOneField(
        get_user_model(), models.CASCADE, related_name="telegram_pairing_token"
    )
    ...
    def __str__(self):
        return f"Telegram pairing token for {self.user}"
